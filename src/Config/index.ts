...
import { type Config, configZod } from "../Interfaces/Config";
...
const parseConfig = (): Config => {
	...
	configData.personalInfo.phoneNumber = parsePhoneNumber(
		configData.personalInfo.phoneNumber,
	);
	let startDate = dayjs(configData.location.daysAround.startDate);
	if (
		!configData.location.daysAround.startDate ||
		!startDate.isValid() ||
...
function parsePreferredDays(preferredDay: string[]): number[] {
	const convertedPreferredDay = preferredDay
		.map((day) => preferredDayList[day.toLowerCase()])
		.filter((e) => e);
...
function parsePersonalInfo(configData: Config) {
	...
	if (!FIRSTNAME || !LASTNAME || !DOB || !EMAIL || !LASTFOURSSN) {
		log.error(
			"Missing environment variables for personal info. Please refer to example.env file.",
		);
		process.exit(1);
	}
