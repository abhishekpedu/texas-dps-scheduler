...
export const getAuthTokenFromBrowser = async () => {
	...
	try {
		...
		await firstNameInput.pressSequentially(config.personalInfo.firstName, {
			delay: _.random(200, 600),
		});
		const lastNameInput = await page.locator(".v-input:nth-child(3) input");
		await lastNameInput.pressSequentially(config.personalInfo.lastName, {
			delay: _.random(200, 600),
		});
		const dobInput = await page.locator(".v-input:nth-child(4) input");
		await dobInput.pressSequentially(
			config.personalInfo.dob.replaceAll("/", ""),
			{ delay: _.random(200, 600) },
		);
		const lastFourSSNInput = await page.locator(".v-input:nth-child(5) input");
		await lastFourSSNInput.pressSequentially(config.personalInfo.lastFourSSN, {
			delay: _.random(200, 600),
		});
		...
		await emailInput.pressSequentially(config.personalInfo.email, {
			delay: _.random(200, 600),
		});
		const verifyEmailInput = await page.locator('input[id="verifyEmail"]');
		await verifyEmailInput.pressSequentially(config.personalInfo.email, {
			delay: _.random(200, 600),
		});
		...
		const captchaTokenPromise = new Promise((resolve, reject) => {
			...
			page.on("response", async (response) => {
				if (
					response.url() ===
						"https://apptapi.txdpsscheduler.com/api/v1/account/auth" &&
					response.status() === 200
				) {
			...
			page
				.waitForSelector(".v-card__actions.text-center > button")
				.then(async () => {
					await page.click(".v-card__actions.text-center > button");
					page
						.waitForSelector(".v-dialog--active")
		...
		const captchaToken = (await captchaTokenPromise) as {
			data: { token: string; eligibleCards: string; existingBookings: string };
		};
		...
		return {
			token: captchaToken.data.token,
			responseId: JSON.parse(captchaToken.data.eligibleCards)[0].ResponseId,
			existBookings: JSON.parse(captchaToken.data.existingBookings),
		};
	} catch (err) {
		log.error("Error while getting captcha token: ", err as Error);
		log.info("Try to get captcha token again");
		process.exit(1);
	}
};
