...
const configZod = z.object({
	personalInfo: z
		...
		// The DPS auth endpoint expects the username in this exact shape, derive it once here.
		.transform((data) => ({
			...data,
			authUserName: `${data.firstName}_${data.lastName}_${data.lastFourSSN}`,
		})),
	location: z.object({
		...
		daysAround: z
			...
			.refine(checkStartLowerThanEnd, {
				message: "Start number must be lower than end number",
			}),
		timesAround: z
			.object({
				start: z.number(),
	...
	appSettings: z.object({
		...
		captcha: z
			...
			.refine(
				...
				{
					message:
						"If you want to use solver, please provide all required fields",
				},
			),
		pushNotification: z
			.object({
			...
			.refine(
				...
				{
					message:
						"If you want to enable push notification, please provide all required fields",
				},
			),
	}),
});
